...
abstract class SHDRecursiveTask
    ...
    protected Map<Integer, BitSet> updateCritAndUncov(Integer v) {
        // Input validation
        if (S.get(v)) {
            throw new IllegalArgumentException("update crit S already contains v.");
        }

        if (!crit.get(v).isEmpty()) {
            throw new IllegalArgumentException("updatecit crit[v] is not empty.");
        }
    ...
    protected void restoreCritAndUncov(Map<Integer, BitSet> critMark,
        ...
        // Input validation
        if (S.get(v)) {
            throw new IllegalArgumentException("restoreCriS still contains v.");
        }

        if (uncov.intersects(crit.get(v))) {
            throw new IllegalArgumentException("restoreCriv is critical for an uncovered edge.");
        }
        ...
        for (int w = S.nextSetBit(0); w >= 0; w = S.nextSetBit(w+1)) {
            if (!critMark.containsKey(w)) {
                throw new IllegalArgumentException("restoreCriw is not in critMark.");
            }
